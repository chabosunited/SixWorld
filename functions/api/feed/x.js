...
export async function onRequestGet({request,env}){
  ...
  return json({items:(d.data||[]).map(t=>({id:`x_${t.id}`,title:t.text,date:t.created_at,source:'X',image:media[t.attachments?.media_keys?.[0]]||'assets/gta-6-trailer-2.jpg',summary:'Rockstar Games on X',url:`https://x.com/${username}/status/${t.id}`}))});
}
